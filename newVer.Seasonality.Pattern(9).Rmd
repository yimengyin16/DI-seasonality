...
Panel_unadj     <- filter(Panel,date_yearmon <= "2020-2")
...
states_order0 <- c(setdiff(states_order0, c("NW", "A9", "A8")))
...
mod_ets <- list(
  ...
  emplyMon_tot          = df_model %>% rename(y = emplyMon_tot)     %>% model(emplyMon_tot = ETS(update(ets_fml_MM, y ~.))),

  emplyMon_gov          = df_model %>% rename(y = emplyMon_gov)     %>% model(emplyMon_gov = ETS(update(ets_fml_MM, y ~.))),
  emplyMon_leisure      = df_model %>% rename(y = emplyMon_leisure) %>% model(emplyMon_leisure = ETS(update(ets_fml_MM, y ~.))),
  emplyMon_trade        = df_model %>% rename(y = emplyMon_trade)   %>% model(emplyMon_trade = ETS(update(ets_fml_MM, y ~.))),

  emplyMon_construction = df_model %>% rename(y = emplyMon_construction) %>%
    filter(!State %in% c("DE", "HI")) %>%
  ...
  dlemplyMon_tot          = df_model %>% rename(y = dlemplyMon_tot) %>% filter(!is.na(y)) %>% model(dlemplyMon_tot = ETS(update(ets_fml_AA, y ~.))),
 
  dlemplyMon_gov          = df_model %>% rename(y = dlemplyMon_gov) %>% filter(!is.na(y))     %>% model(dlemplyMon_gov = ETS(update(ets_fml_AA, y ~.))),
  dlemplyMon_leisure      = df_model %>% rename(y = dlemplyMon_leisure) %>% filter(!is.na(y)) %>% model(dlemplyMon_leisure = ETS(update(ets_fml_AA, y ~.))),
  dlemplyMon_trade        = df_model %>% rename(y = dlemplyMon_trade) %>% filter(!is.na(y))   %>% model(dlemplyMon_trade = ETS(update(ets_fml_AA, y ~.))),
...
states_order <- c("US", setdiff(states_order, c("AG", "NW", "A9", "A8", regionNames)),
...
ggsave(fig_UIDI_US, filename = paste0(dir.fig_newVer, "fig_season_UIDI_UL.png"), width = 7 , height = 7)
