...
states_order0 <- c(setdiff(states_order0, c("A9", "A8")))
...
mod_ets <- list(
  ...
  UIIC = df_model %>% rename(y = UIIC) %>%  filter(!State %in% c("DC", "NW")) %>% filter(!is.na(y))  %>% model(UIIC = ETS(update(ets_fml_MM, y ~.))),
  UICC = df_model %>% rename(y = UICC) %>%  filter(!State %in% c("DC", "NW")) %>% filter(!is.na(y))  %>% model(UICC = ETS(update(ets_fml_MM, y ~.))),
