...
# 3. **paper** Echo effects of COVID-19 
...
m.SSDI.SEATSm.tcao <- seas(df_x13$SSDI, list = spec.SEATSm.tcao)
...
#4. **paper** Potential seasonal changes
...
# 2. Automatic X11
m2.SSDI.x11m.auto = seas(df_x13a$SSDI, x11 = "", transform.function = "log", outlier.types = c("ALL"))
m2.SSI.x11m.auto  = seas(df_x13a$SSI,  x11 = "", transform.function = "log", outlier.types = c("ALL"))
m2.Concurrent.x11m.auto = seas(df_x13a$Concurrent, x11 = "", transform.function = "log", outlier.types = c("ALL"))
...
out.SSDI.X11.auto   <- get_SAimpact("SSDI", "X11", "auto", 2011)
out.SSDI.X11.tcao   <- get_SAimpact("SSDI", "X11", "tcao", 2011)
out.SSDI.SEATS.auto <- get_SAimpact("SSDI", "SEATS", "auto", 2011)
out.SSDI.SEATS.tcao <- get_SAimpact("SSDI", "SEATS", "tcao", 2011)

out.SSI.X11.auto   <- get_SAimpact("SSI", "X11", "auto", 2011)
out.SSI.X11.tcao   <- get_SAimpact("SSI", "X11", "tcao", 2011)
out.SSI.SEATS.auto <- get_SAimpact("SSI", "SEATS", "auto", 2011)
out.SSI.SEATS.tcao <- get_SAimpact("SSI", "SEATS", "tcao", 2011)

out.Concurent.X11.auto   <- get_SAimpact("Concurrent", "X11", "auto", 2011)
out.Concurent.X11.tcao   <- get_SAimpact("Concurrent", "X11", "tcao", 2011)
out.Concurent.SEATS.auto <- get_SAimpact("Concurrent", "SEATS", "auto", 2011)
out.Concurent.SEATS.tcao <- get_SAimpact("Concurrent", "SEATS", "tcao", 2011)
...
df_impact %>%  
  filter(DItype == "SSDI",
         olType == "tcao") %>% 
  ungroup %>%
  ggplot(aes(x = current_year, y = seas_chg, color = method)) + theme_bw() +
  facet_wrap(mon~., nrow = 3) +
  geom_line() +
  geom_point() +
  geom_hline(yintercept = 0, linetype = 2, color = "grey50")+
  geom_vline(xintercept = 2019, linetype = 2, color = "grey50") +
  labs(title = "SSDI")
