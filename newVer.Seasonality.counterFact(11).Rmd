...
```{r}
# What's new in ver(8)
#  - focus on reconcile figures for potential seasonality changes. 

# What's new in ver(9)
#  - change the way to measure potential seasonality changes
#    - In the incremental figure, we now show for each month the difference between 
#      (1) the seasonal factor estimated using data up to the current year t
#      (2) the average seasonal factor of the past two years, which are estimated
#          using data up to year t - 1. 

# What's new in ver(10)
#  - Comparing pre and post pandemic seasonality without Aug 2020 mannually added
#    as an outlier. 
#     - Will it be automatically detected? 
#     - If not modeled as an outlier, how will the Aug seasonal factor be affected?

# What's new in ver(11)
#  - Use automatic outlier detection in the comparison of seasonality
...
fcst_SSDI       <- fabletools::forecast(fit_SSDI, h = 26)
fcst_SSI        <- fabletools::forecast(fit_SSI,  h = 26)
fcst_concurrent <- fabletools::forecast(fit_concurrent, h = 26)
...
autoplot(fcst_SSDI,
  ...
  labs(title = paste0("SSDI only applications"),
       subtitle = "ETS forecast (blue) vs actual (red)",
       x = NULL,
       y = NULL) +
...
# Choose X11 final filter, will be applied to all X11 procedures in this section
# May want to examine robustness if use X13 default setting
# slc_seasonalma <- "x11default" #"s3x3"
# Note: use X11 default auto procedure after 6/4/2022. Because we will not
# include a comparision to CiSSA results. 
...
covidMon_labels <- c(paste0(month.abb[c(3,5, 7, 9, 11)], " 2020"), 
										 paste0(month.abb[c(1, 3, 5, 7, 9, 11)], " 2021"),
										 paste0(month.abb[c(1, 3)], " 2022")
...
df_x13a <- 
  Panel_all %>% 
  filter(State == "NW") %>% 
  filter(date_yearmon <= as.yearmon("2019-12")) %>% 
  select(State, date_yearmon, Formatted.Date, SSDI = Title.2, SSI = Title.16, conc = Concurrent) %>% 
  mutate(SSDI  = ts(SSDI, start = c(2000,10), frequency = 12),
         SSI   = ts(SSI, start = c(2000,10),  frequency = 12),
         Concurrent  = ts(conc, start = c(2000,10), frequency = 12)
...
df_seas_base <- 
  df_seasOut_preCovid %>% 
	filter(date_yearmon >= as.yearmon("2019-01"),
         date_yearmon <= as.yearmon("2019-12")) %>% 
...
df_compareSeas <- 
  ...
  mutate(method = paste0(SAmethod,"_", olType),
         seasType = factor(seasType, c("postCovid", "preCovid"),
                                     c(
                                       "Post-COVID seasonal factor\n(based on data up to 2022:4)",
                                     	 "Pre-COVID seasonal factor of the corresponding month \n(2019 values based on data up to 2020:2)"
                                       )
                           ))

## Figures for papers

## Figure X11 only, automatic outlier detection during COIVD-19 
method_slc1 <- 
  c(X11_auto = "X11",  #"X11\n(TC in 2020:5; AO in 2021:8)",
    SEATS_tcao = "SEATS", #"SEATS\n(TC in 2020:5; AO in 2021:8)",
    CiSSA_none = "CiSSA"
    )

fig_caption <- c("Notes:")

fig_compareSeas_X11auto <- 
  df_compareSeas %>% 
  mutate(date = as_date(date_yearmon)) %>% 
  filter(method %in% names(method_slc1[1])) %>% 
  mutate(method = factor(method, names(method_slc1[1]), method_slc1[1])) %>% 
  ggplot(aes(date, seasonal, color = seasType)) + theme_bw() +
  facet_grid(DItype ~.) +
  geom_line() +
  geom_point(size = 0.9) +
  geom_hline(yintercept = 1, color = "gray50", linetype = 2,size = 0.5) +
	annotate("rect", xmin = as_date('2022-01-01'), xmax = as_date('2022-04-01'), ymin = -2, ymax = 2,
           alpha = .12,fill = "blue") + 
	annotate("rect", xmin = as_date('2021-01-01'), xmax = as_date('2021-12-31'), ymin = -2, ymax = 2,
           alpha = .08,fill = "blue") + 
	annotate("rect", xmin = as_date('2020-03-01'), xmax = as_date('2020-12-31'), ymin = -2, ymax = 2,
           alpha = .04,fill = "blue") + 
	annotate("text", x = c(as_date('2020-08-01')), y = c(1.12), label = c("2020") ) + 
	annotate("text", x = c(as_date('2021-07-01')), y = c(1.12), label = c("2021") ) + 
	annotate("text", x = c(as_date('2022-02-15')), y = c(1.12), label = c("2022") ) + 
  coord_cartesian(ylim = c(0.87, 1.13)) + 
  scale_color_manual(values = c("firebrick2","grey40")) + 
  scale_x_date(breaks = as_date(c("2020-03-01", "2020-08-01","2021-01-01", "2021-03-01", "2021-08-01", "2022-01-01")),
               labels = c("Mar 2020", "Aug 2020", "Jan 2021", "Mar 2021", "Aug 2021", "Jan 2022")
               ) + 
  labs(x = NULL,
       y = "Seasonal component (based on X11 method)",
       color = NULL
       #caption = fig_caption
  		 ) +
  theme(legend.position = "bottom",
...
ggsave(fig_compareSeas_X11auto, filename = paste0(dir.fig_newVer, "fig_covid_compareSeas_X11auto.png"),
...
ggsave(fig_compareSeas_CiSSA_L48, filename = paste0(dir.fig_newVer, "fig_covid_compareSeas_CiSSA_L48.png"),
       width = 10*0.8, height = 10*0.8)



## Figure without CiSSA
fig_caption <- c("Notes: - In both methods, May 2020 is hardcoded as a temporary change (TC) and Aug 2021 is hardcoded as a additive outlier (AO).")

fig_compareSeas_noCiSSA <- 
  df_compareSeas %>% 
  mutate(date = as_date(date_yearmon)) %>% 
  filter(method %in% names(method_slc[1:2])) %>% 
  mutate(method = factor(method, names(method_slc[1:2]), method_slc[1:2])) %>% 
  ggplot(aes(date, seasonal, color = seasType)) + theme_bw() +
  facet_grid(DItype ~ method) +
  geom_line() +
  geom_point(size = 0.9) +
...
ggsave(fig_compareSeas_noCiSSA, filename = paste0(dir.fig_newVer, "fig_covid_compareSeas_noCiSSA.png"),
       width = 10*0.8, height = 10*0.8)
...
fig_compareSeas_X11 <- 
  df_compareSeas %>% 
  mutate(date = as_date(date_yearmon)) %>% 
  filter(method %in% names(method_slc[1])) %>% 
  mutate(method = factor(method, names(method_slc[1]), method_slc[1])) %>% 
  ggplot(aes(date, seasonal, color = seasType)) + theme_bw() +
  facet_grid(DItype ~.) +
  geom_line() +
  geom_point(size = 0.9) +
...
ggsave(fig_compareSeas_X11, filename = paste0(dir.fig_newVer, "fig_covid_compareSeas_X11.png"),
       width = 10*0.8, height = 10*0.8)
...
# function to get seasonal adjustment results
...
# function to conduct seasonal adjustment incrementally
...
if(SAmethod == "X11") {
	spec$x11 = ""
	#spec$x11.seasonalma = "x11default" # "s3x3"
}

# Defining outliers under different settings
...
if(current_year == 2020 & olType == "tcao"){
  spec$regression.variables = c("tc2020.may")
}
...
if(current_year == 2020 & olType == "lsao"){
  spec$regression.variables = c("ls2020.may")
}
...
pb <- progress_bar$new(total = length(start_year:2022))

df_allyears <- map_dfr(start_year:2022, function(x) doSA(x, df))
...
# df_allyears %>% filter(current_year %in% c(2020, 2021), mon == 5, year >= 2018)


# Calculate the revision to the previous year
# df_preYear <- 
#   df_allyears %>% 
...
# 	mutate(seas_chg = seasonal - seasonal_preYr) %>% 
#   filter(year >= (current_year - 1),
#          current_year != year)
# 
# # df1 %>% filter(current_year %in% c(2020, 2021), mon == 4 )
# 
# df2 <- 
#   df1 %>% 
...
# Calculate the difference between the seasonal factors estimated using data
#  up to year t and those estimated using data up to year t - 1 (average of t - 1 and t - 2)

# Seasonal factor estimated using data up to year t
df_current <- 
  df_allyears %>%
	filter(year == current_year) %>% 
	arrange(current_year, mon) %>% 
  select(date_yearmon, current_year, mon, seasonal_current = seasonal)


# Seasonal factor estimated using data up to year t - 1
df_pre <- 
  df_allyears %>%
	# filter(year == current_year |
	# 			 year == current_year - 1) %>%
	filter(year == current_year) %>% 
	group_by(current_year, mon) %>% 
	summarize(seasonal_pre = mean(seasonal)) %>% 
	arrange(current_year, mon) %>% 
	mutate(current_year = current_year + 1)

df1 <-
  df_current %>%
  left_join(df_pre, by = c("current_year", "mon")) %>%
  filter(current_year > start_year) %>%
	mutate(seas_chg = seasonal_current - seasonal_pre)
...
df_fig.impact_auto <- 
  ...
  filter(olType == "auto") %>% 
  mutate(method = factor(method, c("X11.auto"),
                                 c("X11" )))




# Function to plot the changes in seasonal factor from the previous year
plot_impact <- function(DIType_slc, method_slc, fill_slc = "blue", Data = df_fig.impact){
# DIType_slc <- "SSDI"
# method_slc <- "X11"

	
  Data%>% 
  filter(DItype == DIType_slc,
  			 method == method_slc, 
  			 current_year >= 2006,
  			 !(current_year == 2022 & mon %in% month.abb[5:12])
  			 ) %>% 
  ungroup %>%
  ggplot(aes(x = current_year, y = seas_chg)) + theme_bw() +
  facet_wrap(mon~., nrow = 2) +
  #geom_line() +
  #geom_point() +
  geom_bar(stat = "identity", position ="dodge", width = 0.7, fill = fill_slc) + 
  ...
  geom_vline(xintercept = 2019.5, linetype = 2, color = "grey50") +
	annotate("rect", xmin = 2019.5, xmax = 2022.5, ymin = -1, ymax = 1,
           alpha = .1,fill = "blue") + 
  labs(title = DIType_slc,
       x = NULL,
       y = "Change in seasonal factor") + 
  theme(legend.position = "bottom",
...
# For X11 auto
fig_impact_SSDI_x11auto       <- plot_impact("SSDI", "X11", Data = df_fig.impact_auto)
fig_impact_SSI_x11auto        <- plot_impact("SSI", "X11",  Data = df_fig.impact_auto)
fig_impact_Concurrent_x11auto <- plot_impact("Concurrent", "X11", Data = df_fig.impact_auto)

fig_impact_SSDI_x11auto
fig_impact_SSI_x11auto
fig_impact_Concurrent_x11auto 

fig_impact_3types_x11auto <- 
grid.arrange(fig_impact_SSDI_x11auto,
						 fig_impact_SSI_x11auto,
						 fig_impact_Concurrent_x11auto,
						 nrow = 3,
						 top = textGrob("Changes in seasonal factor compared to the previous year"))


ggsave(fig_impact_3types_x11auto, filename = paste0(dir.fig_newVer, "fig_impact_3types_x11auto.png"), height = 10, width = 9, scale = 1)
...
# For X11 tcao
fig_impact_SSDI_x11       <- plot_impact("SSDI", "X11")
fig_impact_SSI_x11        <- plot_impact("SSI", "X11")
fig_impact_Concurrent_x11 <- plot_impact("Concurrent", "X11")

fig_impact_SSDI_x11
fig_impact_SSI_x11
fig_impact_Concurrent_x11 

fig_impact_3types_x11 <- 
grid.arrange(fig_impact_SSDI_x11,
						 fig_impact_SSI_x11,
						 fig_impact_Concurrent_x11,
						 nrow = 3,
						 top = textGrob("Changes in seasonal factor compared to the previous year\n Seasonal adjustment procedure: X11"))


ggsave(fig_impact_3types_x11, filename = paste0(dir.fig_newVer, "fig_impact_3types_x11.png"), height = 10, width = 9, scale = 1)


# For SEATS method
fig_impact_SSDI_SEATS       <- plot_impact("SSDI", "SEATS", "firebrick3")
fig_impact_SSI_SEATS        <- plot_impact("SSI", "SEATS", "firebrick3")
fig_impact_Concurrent_SEATS <- plot_impact("Concurrent", "SEATS", "firebrick3")


fig_impact_SSDI_SEATS
fig_impact_SSI_SEATS
fig_impact_Concurrent_SEATS 

fig_impact_3types_SEATS <- 
grid.arrange(fig_impact_SSDI_SEATS,
						 fig_impact_SSI_SEATS,
						 fig_impact_Concurrent_SEATS,
						 nrow = 3,
						 top = textGrob("Changes in seasonal factor compared to the previous year\n Seasonal adjustment procedure: SEATS"))


ggsave(fig_impact_3types_SEATS, filename = paste0(dir.fig_newVer, "fig_impact_3types_SEATS.png"), height = 10, width = 9, scale = 1)
