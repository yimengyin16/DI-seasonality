...
ThreeSeries.NW = ggplot(AppSeries, aes(x = Formatted.Date, y = value, colour = variable)) + theme_bw() + 
              ...
              scale_x_date(date_breaks = "1 year", labels = date_format("%Y"), limits = as.Date(c("2000-10-1", "2022-4-1"))) + 
...
ggsave(fig_UIDI_NW, filename = paste0(dir.fig_newVer, "fig_season_UIDI_US+.png"), width = 7 , height = 7)
...
df_season_avg %>% 
  ...
  ungroup() %>% 
  ggplot(aes(x = (Month), y = season_avg)) + theme_bw() + facet_wrap(.~ .model, ncol = 3) + 
  geom_boxplot(outlier.size = 1) + 
  geom_hline(yintercept = 1, linetype = 2, color = "grey70") + 
  coord_cartesian(ylim = c(0.75, 1.25)) + 
  labs(x = NULL,
...
ggsave(fig_season_boxplot_v, filename = paste0(dir.fig_newVer, "fig_season_boxplots.png"),
       width = 7.7, height = 9.9) 

  
ggsave(fig_season_boxplot_h, filename = paste0(dir.fig_newVer, "fig_season_boxplots_h.png"),
       width = 14, height = 4.5, scale = 0.75) 
...
states_north <- c("AK", "MN", "NY", "ME")
states_south <- c("AZ", "TX", "LA", "FL")


## SSDI and unemployment 

fig_unempSSDI_northSouth <- 
  df_season_avg %>% 
  filter(State  %in% c(states_north, states_south),
        .model %in% c("unemply", "SSDI")
        ) %>% 
  # lag unemployment by 2 months
  mutate(Month_lag = ifelse(.model == "unemply", 
         ...
         # Month_lag = str_sub(Month_lag, 1),
         State = factor(State, c(states_north, states_south)),
         NorthSouth = ifelse(State %in% states_north, "Northern states", "Southern states")
         ) %>% 
  mutate(.model = factor(.model, 
                         c("unemply", "SSDI"), 
                         c("Unemployment (2-month lag)", "SSDI only application"))) %>% 
  ...
  geom_point(size = 1.5) + 
  geom_hline(yintercept = 1, color = "grey70") + 
  geom_hline(yintercept = c(1.1, 0.9), color = "grey70", linetype = 2) + 
  #coord_cartesian(ylim = c(0.75, 1.25)) + 
   coord_cartesian(ylim = c(0.7, 1.35)) + 
  scale_x_continuous(breaks = 1:12, labels = str_sub(month_lab, 1, 1)) + 
  scale_y_continuous(breaks = seq(0, 2, 0.1)) + 
  scale_color_manual(values = c("black", "blue")) + 
  labs(# title = slc,
       x = NULL,
...
## UI and unemployment 
fig_unempUI_northSouth <- 
  df_season_avg %>% 
  filter(
         State  %in% c(states_north, states_south),
        .model %in% c("unemply", "UIIC")
        ) %>% 
  mutate(.model = factor(.model, 
                         c("unemply", "UIIC"), 
...
fig_unempSSDI.UI_northSouth <- 
  df_season_avg %>% 
  filter(State  %in% c(states_north, states_south),
        .model %in% c("unemply", "SSDI", "UIIC")
        ) %>% 
  # lag unemployment by 2 months
  mutate(Month_lag = ifelse(.model == "unemply", 
                        ifelse(Month > 10, Month - 12 + 2,
  ...
  mutate(.model = factor(.model, 
                         c("unemply", "SSDI", "UIIC"), 
                         c("Unemployment (2-month lag)", "SSDI only application", "UI initial claim"))) %>% 
  arrange(.model) %>% 
  ggplot(aes(x = Month_lag, y = season_avg, color = .model)) + theme_bw() + 
  facet_wrap(. ~ State, nrow = 2) + 
...
ggsave(fig_unempSSDI_northSouth, filename = paste0(dir.fig_newVer, "fig_northSouth_SSDI.png"),
       width = 12, height = 7, scale = 0.9) 

  
ggsave(fig_unempUI_northSouth, filename = paste0(dir.fig_newVer, "fig_northSouth_UI.png"),
       width = 12, height = 7, scale = 0.9) 



  
ggsave(fig_unempSSDI.UI_northSouth, filename = paste0(dir.fig_newVer, "fig_northSouth_SSDI.UI.png"),
       width = 12, height = 7, scale = 0.85) 
