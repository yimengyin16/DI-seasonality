...
# Seasonal patterns using models, single state
...
#   ggplot(aes(x = as.yearmon(Formatted.Date), y = UIIC)) + 
#   geom_line()
# 
# 
# panel_ts %>% 
...
#   filter(State == "AG") %>% 
#   filter(date_yearmon <= yearmonth("2020-02")) %>% 
#   ggplot(aes(x = as.yearmon(Formatted.Date), y = dlunemply_U)) + 
#   geom_line()
...
mod_ets_UIIC <-  # number of UI initial claims
  df_model_AG %>% 
  model(mod_ets = ETS(update(ets_fml_MM, UIIC ~.)))

mod_ets_UICC <-  # number of UI continued claims
  df_model_AG %>% 
  model(mod_ets = ETS(update(ets_fml_MM, UICC ~.)))
...
# UI initial
components(mod_ets_UIIC) %>% gg_season(season, label = "both")

# UI continued
components(mod_ets_UICC) %>% gg_season(season, label = "both")
...
components(mod_ets_UIIC) %>% autoplot()
components(mod_ets_UICC) %>% autoplot()
...
# Seasonal patterns using models, for paper
